 ...
 * @author chouhl
 */
public class ExeTaskMainCtrl1 implements IController {
	...
	protected void onBeforeSubmit(){
		...
		if(WfmConstants.WfmOperator_BeforeAddSign.equalsIgnoreCase(operator) && StringUtils.isBlank(userPks)){
...
//		WfmUtilFacade.isExistFloInf();
		...
		if(!"true".equalsIgnoreCase(isCleared)){
...
//		LfwView currWidget = appCtx.getCurrentWindowContext().getCurrentViewContext().getView(); 
...
//					AppInteractionUtil.showMessageDialog(scratchPad);
	...
	public void attachClick(LinkEvent linkEvent) {
		...
		if (StringUtils.isNotBlank(taskPk)) {
			...
			if (StringUtils.isNotBlank(tbillitem) && !tbillitem.equals(billitem) ) {
...
//			textComp.setValue(value);
	...
	public void textValueChanged(TextEvent textEvent) {
		...
		if (WfmConstants.WfmOperator_Agree.equalsIgnoreCase(operator)) {
			this.setAgreeState();
			((UICardLayout)uimeta.findChildById("card1")).setCurrentItem("0");
			...
			((UICardLayout)uimeta.findChildById("card1")).setCurrentItem("1");
			...
			((UICardLayout)uimeta.findChildById("card1")).setCurrentItem("2");
...
//			panel4.setVisible(false);
		...
		ViewContext viewContext = AppUtil.getViewCtx(getPubView());
		if (viewContext!=null) {

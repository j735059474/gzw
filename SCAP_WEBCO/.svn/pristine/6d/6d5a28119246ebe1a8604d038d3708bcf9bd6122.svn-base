...
public class UifAfterMainRowSelectAddSubFormCmd extends UifCommand {
	...
	public void execute() {
		...
		if (this.dsId == null)
			throw new LfwRuntimeException(NCLangRes4VoTransl.getNCLangRes().getStrByID("pub", "UifAddCmd-000000")/*未指定数据集id!*/);
		Dataset ds = widget.getViewModels().getDataset(dsId);
		if (ds == null)
			throw new LfwRuntimeException(NCLangRes4VoTransl.getNCLangRes().getStrByID("pub", "UifAddCmd-000001")/*数据集为空,数据集id=*/ + dsId + "!");
	...
	protected void setNavPkToRow(Row row, String navId, Dataset slaveDs) {
		...
		if (dsRels != null) {
			...
			if (navrow == null) {
				throw new LfwRuntimeException(NCLangRes4VoTransl.getNCLangRes().getStrByID("pub", "UifAddCmd-000002")/*请选择导航数据集*/);
			}

...
//		return;
	...
	if (rowData != null)
		rowData.notifyChange(NotifyType.FOCUS,this.focusRowIndex);
};
...
Dataset.prototype.isDataRequested = function(keyValue, index) {
	...
	if(index==null || typeof(index)=="undefined")
		pageIndex = rowSet.getPageIndex();
	if (pageIndex == -1)
		return true;
	//处理当前页数据条数超过pageSize的情况，返回true      
		...
		for ( var i = 0; i < records.length; i++) {
			...
			for ( var j = nindex; j < oldRows.length; j++) {
				if (oldRows[j].rowId == records[i].rowId) {
					...
					if (oldRows[j].state != 0 && pageUI != null)
...
RowData.prototype.setValueAt = function(rowIndex, colIndex, value) {
	...
	// 下面的代码会导致form中修改当前语种时结果只修改了主语种，当前语种没有变化。因此注释掉。
	// grid的问题应通过修改grid的渲染解决
	/*// 改当前语种时，获得grid绑定的字段
	var fieldBinded = this.isFieldExsit(colIndex);
	var fieldBindedColIndex = dataset.nameToIndex(fieldBinded);
	var value = row.getCellValue(fieldBindedColIndex);
	if(fieldBinded != null){
		var event = new DataChangeEvent();
		event.currentRow = row;
		event.cellRowIndex = rowIndex;
		event.cellColIndex = fieldBindedColIndex;
		event.currentValue = value;
		...
		if (!dataset.silent) {
			dataset.dispatchEvent(event);
			//  校验cell数据
			dataset.checkDatasetCell(value, dataset.nameToIndex(fieldBinded), row);
			// 对外暴露函数
			dataset.onAfterDataChange(event);
		}
	}*/

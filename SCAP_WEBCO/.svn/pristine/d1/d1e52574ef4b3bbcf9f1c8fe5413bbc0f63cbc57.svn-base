...
public class QywhzlPortlet extends PtBasePortlet {
...
//		String newsType = request.getPreferences().getValue("newsType", "5");//动态获取portlet属性newsType
		...
		NewsVO[] result = (NewsVO[]) ScapDAO.retrieveByCondition(NewsVO.class, " type = '7' and formstate = 'End' ", " ts desc ");
...
//		if(result.size() > 0){
			try {
				...
				String template = "portlets/QywhzlPortlet.html";

...
import nc.scap.pub.vos.ScapChartVO;
 ...
 * 
 */
public class DefineChartCardWinMainViewCtrl <T extends WebElement> extends AbstractMasterSlaveViewController{
	...
	public void onSave(MouseEvent<?> mouseEvent) throws BusinessException {
		Dataset masterDs = this.getMasterDs();
		Object pkid = masterDs.getSelectedRow().getValue(masterDs.nameToIndex(ScapChartVO.PK_CHART));
		Object code = masterDs.getSelectedRow().getValue(masterDs.nameToIndex(ScapChartVO.CODE));
		if(code != null) {
			String condition = "code = '" + code.toString() + "'";
			if(pkid != null) {
				condition += " and pk_chart != '" + pkid.toString() + "'";
			}
			ScapChartVO[] vos = (ScapChartVO[]) ScapDAO.retrieveByCondition(ScapChartVO.class, condition);
